...
def build_model(model_name: str, num_classes: int = 1):
    """Factory to build different backbones for binary classification."""
    import torch.nn as nn
...
def main():
    ...
    history = train_1_binary(
        ...
        early_stopping_patience=args.early_stopping_patience,   # <-- use the flag here
    ...
    test_acc = (np.array(y_pred).squeeze() == np.array(y_true)).mean()
